...
use crate::frontend::{calendar::Calendar, jamselector::JamSelector, track_list::TrackList};
...
fn HomePage() -> impl IntoView {
    ...
    view! {
        ...
        <Calendar></Calendar>
        <JamSelector selected_day=selected_day set_selected_jam_id=set_selected_jam_id></JamSelector>
        <TrackList selected_jam_id=selected_jam_id/>
        <Show
            when=move || {!started.get()}
            fallback=|| view! {<p>"Pipeline started"</p>}>
        <button on:click=move |_| {
            spawn_local(async 
                {
                    //create_pipeline("meme".to_string()).await;
                });
        ...
        <button on:click=move |_| {
            spawn_local(async 
                {
                    //play_pipeline("meme".to_string()).await;
                });
            }>
        "Play pipeline"
        </button>
